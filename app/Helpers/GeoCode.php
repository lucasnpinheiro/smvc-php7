...
class GeoCode {
	 ...
	 *
	 * @param array $options
	 *        	should contain up to 4 keys for steet,city,state and zipcode
	 *        	
	...
	public static function getLngLat(array $options) {
		...
		$address = array ('street' => $options[0],'city' => $options[1],'state' => $options[2],'zipcode' => $options[3]);
		$url = $url_base . urlencode($address['street']) . ',' . urlencode($address['city']) . ',' . $address['state'] . ',' . $address['zipcode'] . $url_end;
		
		$result = json_decode(file_get_contents($url), true);
		
		if ($result['status'] == 'OK') {
			return array ('lon' => $result['results'][0]['geometry']['location']['lng'],'lat' => $result['results'][0]['geometry']['location']['lat']);
		} else {
