...
class Watchlist {
    ...
    public static function printList($clearStack = true) {
        ...
        if (count($stack) > 0) {
            ...
            echo '</table>'
            . '<script>jQuery(\'.Helpers_Watchlist_Table_Rows_' . $randomNumber . '\').toggle()</script>';
    ...
    public static function trace($levels = 10, $label = null) {
        ...
        for ($i = 1; $i <= $levels; $i++) { //$i>0, so that this method call would not be traced
            if ($debugBacktrace [$i]['file'] == '')
                continue;
            $args = (array) $debugBacktrace [$i]['args'];

            $backtraceString.='<br /> ' . $debugBacktrace [$i]['file'] . ' [' . $debugBacktrace [$i]['line'] . ']: '
                    . $debugBacktrace [$i]['function'] . '(' . print_r(implode(', ', $args), true) . ') ';
        }
