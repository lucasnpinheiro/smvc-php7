...
class TableBuilder {
	...
	/**
	 *
	 * @var \core\database A database instance
	 */
	protected $db;
	 ...
	 */
	private static $typeAliases = array ('int' => 'INT(11)','string' => 'VARCHAR(255)','description' => 'TINYTEXT');
	 ...
	 *
	 * @param int|array $constant
	 *        	- Constant(s) to convert
	 * @return string
	 */
	 ...
	 *        	- Field name
	 * @param string $type
	 *        	- Type of the field, for types, please visit CREATE TABLE page for reference
	 * @param boolean $null
	 *        	- NOT null or null
	 * @param array|int $options
	 *        	- Options, it's either array of constants or just one constant
	 */
	public function addField($field, $type, $null = false, $options = 0) {
		...
		$this->fields[$field] = array ('type' => $type,'null' => $null,'options' => $options);
		
		if ($options === self::CURRENT_TIMESTAMP || is_array($options) && in_array(self::CURRENT_TIMESTAMP, $options)) {
	 ...
	 *
	 * @param string $field
	 *        	- Field which you want to set a primary key
	 * @return boolean
	 */
	 ...
	 *
	 * @param boolean $reset
	 *        	- A flag to reset whole set of data.
	 * @return boolean
	 */
